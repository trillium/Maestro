/**
 * AgentConfigPanel — webFull lift
 *
 * Verbatim port of `src/renderer/components/shared/AgentConfigPanel.tsx`
 * (696 LOC, pure presentational — zero IPC, zero Electron-only APIs).
 * Lifted as a leaf dependency of `NewInstanceModal`'s webFull port (the
 * "create a new agent" entry point, the biggest single-modal user-felt
 * unlock per the leaf-parade brief 2026-06-08).
 *
 * Shared component for agent configuration settings. Used by both
 * NewInstanceModal and EditAgentModal in this lift; the renderer source
 * is additionally used by the Wizard's AgentSelectionScreen (out of
 * scope here).
 *
 * Displays:
 ...
 * All model lookups, config blurs, and refresh fan-outs route
 * through the prop callbacks (`onConfigBlur`, `onRefreshModels`,
 * `onRefreshAgent`). The only changes vs the renderer:
 *
 * - `Theme, AgentConfig, AgentConfigOption` from `'../../types'` →
 *   `Theme` from `'../../../shared/theme-types'`, and
 *   `AgentConfig, AgentConfigOption` from `'../../../renderer/types'`
 *   (type-only re-import — an established webFull pattern, see
 *   SendToAgentModal.tsx line 108).
 *
 * ## What this file does NOT change vs the renderer source
 *
 * Verbatim: ModelTextInput dropdown filtering + click-outside handling;
 * BUILT_IN_ENV_VARS readout with HelpCircle tooltip; stable env-var IDs
 * via envVarIdsRef Map so focus survives key edits; deferred key edits
 * via pendingKeyEditsRef (only committed on blur); compact-vs-non-compact
 * padding split; readOnly path field when SSH is enabled without an
 * override; configOptions fan-out by option.type (text / number /
 * checkbox / select).
 *
...
import type { AgentConfig, AgentConfigOption } from '../../../renderer/types';
