 ...
 * FirstRunCelebration
 *
 * Lifted from `src/renderer/components/FirstRunCelebration.tsx` as part of
 * the Layer 2.5 leaf-parade wave. Implementation is verbatim except for
 * three import-path adjustments matching the L2.5 precedent:
 * - `Theme` from `'../types'` → `'../../shared/theme-types'` (renderer
 *   routes through `src/renderer/types/index.ts`; webFull imports the
 *   type directly).
 * - `useLayerStack` from `'../contexts/LayerStackContext'` resolves to
 *   the webFull-side context at `src/webFull/contexts/LayerStackContext.tsx`
 ...
 * - `formatShortcutKeys` from `'../utils/shortcutFormatter'` resolves to
 *   the webFull-side shim at `src/webFull/utils/shortcutFormatter.ts`
 *   (precursor infrastructure shipped with the L2.5 `AutoRunnerHelpModal`
 *   lift on branch `leaf-autorunner-help` @ `a82d4012e`). The shim swaps
 *   the renderer formatter's transitive `window.maestro.platform` preload-
 *   bridge dependency for webFull's `navigator.userAgent`-based
 *   `isMacOSPlatform()`. The rest of the formatter (key-map tables,
 *   separator logic, format helpers) is verbatim from the renderer source.
 *
 ...
 *
 * Composition shape: large celebration modal — does NOT compose the L2.1
 * `Modal` primitive directly because the renderer source builds bespoke
 * DOM chrome (dark backdrop, gradient header glow, animated icon, duration
 * pill, encouraging-message card, three-row "what's next" block, gradient
 * dismiss button, optional leaderboard CTA, animated zoom-in / zoom-out
 * transitions, three-burst + delayed star-burst confetti firing on mount
 * AND on close). Layer-stack registration uses `MODAL_PRIORITIES.STANDING_OVATION`
 * (1100 — top tier) with `focusTrap: 'strict'` and routes Escape to
 * `handleClose()`. The "Standing Ovation" variation triggers when the first
 * Auto Run exceeds 15 minutes (STANDING_OVATION_THRESHOLD_MS) and swaps
 * the accent colour to gold, the icon to Trophy, the title text, and adds
 * a delayed gold-star burst.
 *
 * Celebration-on-close is the source-fidelity pattern: clicking the
 * dismiss button (or pressing Enter/Escape) sets `isClosing`, fires a
 * second confetti burst, then calls `onClose()` after a 1000ms animation
 * delay. The Got It / Standing Ovation button label flips to
 * "🎉 Let's Go! 🎉" during the `isClosing` window — these are the only
 * emojis in the file and they come from the renderer source verbatim.
 * Per the brief's hard rule "no emojis", they would normally be stripped
 * from new content; the verbatim-lift rule supersedes here (changing
 * source text would create silent parity drift against the renderer
 * oracle). Documented for the L2.6+ feature-wiring pass that will host
 * the modal — if Trillium wants the emojis removed, that's a renderer-
 * source edit, not a webFull-lift edit.
 */
