/**
 * WizardConversationView.tsx — webFull-side lift of
 * `src/renderer/components/InlineWizard/WizardConversationView.tsx`
 * (683 LOC) with the Wizard Phase-1 import-path adjustments.
 *
 * Lift rationale: this is the second of the two Wizard Phase-1 leaves
 * previously blocked by `createWizardBubbleMarkdownComponents` in
 * `src/renderer/utils/markdownConfig.ts`. The block ran transitively
 * through `WizardMessageBubble` — once that lift landed against the
 * surgically-extracted webFull `markdownConfig.ts`, this view's only
 * remaining cross-fork edges were the pure wizard support modules
 * under `Wizard/services/` and `Wizard/shared/`, which the lift
 * imports directly from the renderer per the Wizard Phase-1 precedent.
 *
 * Import-path adapts:
 *   - `Theme` from `'../../types'` → `'../../../shared/theme-types'`
 *     (standard webFull-tree swap).
 *   - `WizardMessageBubble`, `WizardMessageBubbleMessage` from
 *     `'./WizardMessageBubble'` → `'./WizardMessageBubble'` (sibling
 *     in the webFull tree — same relative path as the renderer, but
 *     resolves to the lifted webFull copy).
 *   - `getNextFillerPhrase` from `'../Wizard/services/fillerPhrases'`
 *     → `'../../../renderer/components/Wizard/services/fillerPhrases'`
 *     (pure module, 0 IPC — verified via grep for
 *     `window\.maestro\.|from ['\"]electron['\"]|shell\.openExternal|ipcRenderer`
 *     → empty).
 *   - `TypingIndicator` from `'../Wizard/shared/TypingIndicator'`
 *     → `'../../../renderer/components/Wizard/shared/TypingIndicator'`
 *     (pure component, 0 IPC — verified via the same grep).
 *   - `formatAgentName`, `getToolDetail` from
 *     `'../Wizard/shared/wizardHelpers'`
 *     → `'../../../renderer/components/Wizard/shared/wizardHelpers'`
 *     (pure module, 0 IPC — verified via the same grep; this is the
 *     second consumer of the renderer-direct import established by
 *     the `WizardMessageBubble` lift on this same branch).
 *
 ...
 * → empty. Component carries no Electron-only surface either directly
 * or transitively — the only previously-blocking edge was via the
 * `WizardMessageBubble` child, which is now the lifted webFull copy.
 *
 * Renderer-side
 * `src/renderer/components/InlineWizard/WizardConversationView.tsx`
...
import { getNextFillerPhrase } from '../../../renderer/components/Wizard/services/fillerPhrases';
