 ...
 * HistoryHelpModal
 *
 * Lifted from src/renderer/components/HistoryHelpModal.tsx as part of the
 ...
 * consistent with the L2.1 Modal/FormInput primitives and the L2.4/L2.5 lifts
 * (ResetTasksConfirmModal, PlaybookNameModal, PlaybookDeleteConfirmModal).
 *
 * Sibling shape: this is a presentational HELP modal — no input fields, no
 * confirm/cancel branching, no destructive action. A single "Got it" footer
 * button calls `onClose`. The body is pure JSX content describing the
 * History panel feature set (entry types, status indicators, detail view,
 * resume sessions, time & cost, activity graph, per-session storage,
 * cross-session memory). 0 IPC namespaces touched. 0 Electron-only APIs
 * touched.
 *
 * Uses `MODAL_PRIORITIES.CONFIRM` (1000) per the renderer source — same
 * priority slot used by other info/help/confirm overlays.
 */
