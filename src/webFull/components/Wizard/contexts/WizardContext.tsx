/**
 * WizardContext.tsx (webFull)
 *
 * Port of `src/renderer/components/Wizard/WizardContext.tsx` per Phase 3A of
 * WIZARD_LIFT_PLAN.md. The state management surface is verbatim — only the
 * persistence channel changed:
 *
 *   Renderer                                webFull
 *   ──────────────────────────────────────  ────────────────────────────────
 *   `window.maestro.settings.get(key)`   →  `settingsStoreWebFull.get(key)`
 *   `window.maestro.settings.set(k, v)`  →  `settingsStoreWebFull.set(k, v)`
 *
 * Both call sites the renderer used to persist `wizardResumeState` (clear via
 * `set(key, null)`, write via `set(key, snapshot)`, read via `get(key)`) map
 * onto `PATCH /api/settings` + `GET /api/settings` per the routes already
 * wired at apiRoutes.ts:1356/1387. The `PATCH` semantics include null-as-
 * clear, matching the `electron-store.set(key, null)` behavior the renderer
 * relied on (verified in WIZARD_LIFT_PLAN.md §357).
 *
 * No other Electron-only imports were present in the source file — the
 * `ToolType` / `AgentConfig` types are pulled from the canonical renderer
 * types module, following the established cross-fork import pattern used by
 * `sessionValidation.ts`, `SendToAgentModal.tsx`, etc.
 *
 * The consumer API (`WizardContextAPI`) is byte-identical so screen
 * components ported in Phase 4 (`AgentSelectionScreen`, `DirectorySelection
 * Screen`, `ConversationScreen`, `PreparingPlanScreen`) need no signature
 * changes.
 *
 * Resume-state writes still fire on every `state.currentStep` change. The
 * plan §378 flags this as a debounce candidate but explicitly says the
 * debounce should land *before* the lift, not as part of it — so the
...
import type { ToolType, AgentConfig } from '../../../../renderer/types';
import { settingsStoreWebFull } from '../../../stores/settingsStore.webfull';
...
/**
 * Persist `wizardResumeState` via the webFull settings store. Fire-and-
 * forget — matches `window.maestro.settings.set` semantics from the
 * renderer. Errors are swallowed because resume state is best-effort
 * (matches the renderer behavior; a failed save just means the user
 * can't resume that particular session).
 */
function persistResumeState(value: unknown): void {
	void settingsStoreWebFull.set('wizardResumeState', value).catch(() => {
		// Best-effort; matches renderer fire-and-forget.
	});
}
...
export function WizardProvider({ children }: WizardProviderProps) {
	...
	const saveStateForResume = useCallback(() => {
		...
		persistResumeState(serializableState);
	...
	const hasResumeState = useCallback(async (): Promise<boolean> => {
		try {
			const saved = await settingsStoreWebFull.get('wizardResumeState');
	...
	const loadResumeState = useCallback(async (): Promise<SerializableWizardState | null> => {
		try {
			...
			if (saved && typeof saved === 'object') {
				...
				const resumeState = saved as SerializableWizardState;
				if (resumeState.currentStep && resumeState.currentStep !== 'agent-selection') {
