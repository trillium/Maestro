/**
 * PhaseReviewScreen.tsx (webFull lift — Phase 2 medium-IPC)
 *
 * Lifted from `src/renderer/components/Wizard/screens/PhaseReviewScreen.tsx`
 * per `ISC-44.lift.wizard_phase_review_screen_lifted`. Document review +
 * launch screen for the onboarding wizard.
 *
 * IPC swaps:
 *
 *   1. `window.maestro.autorun.writeDoc(folderPath, filename, content)` (×3)
 *      → `POST /api/autorun/write-doc` with `{path: <folderPath>/<filename>,
 *      content}`. Mirrors AutoRun's `fetchWriteDoc` byte-for-byte (route
 *      flattens the tuple to a single absolute path; mkdir -p's on demand).
 *
 *   2. `window.maestro.autorun.deleteImage(folderPath, filename)` (×1) →
 *      `DELETE /api/autorun/delete-image?folderPath=…&relativePath=…`. Mirrors
 *      AutoRun's `fetchDeleteImage`. Note the parameter rename
 *      `filename` → `relativePath` matches the route contract; both refer to
 *      the same string the renderer-side `autorun:deleteImage` IPC reply
 *      accepted as its second argument.
 *
 * Cross-fork edges (runtime hook + type/constant, established precedent):
 *
 *   - `useWizard, WizardContext` runtime hook from
 *     `'../../../../renderer/components/Wizard/WizardContext'` — required for
 *     state access (`state`, `setEditedPhase1Content`, `getPhase1Content`,
 *     `setWantsTour`, `setCurrentDocumentIndex`, `previousStep`). The hook
 *     resolves at compile time; at runtime it needs the `WizardProvider` in
 *     the React tree. webFull does not yet mount the orchestrator
 *     (`MaestroWizard`), so this screen is wire-ready but not mountable until
 *     Phase 3. This matches the index.ts note "heavy screens, and service
 *     engines remain in renderer-only until Phase 3" — the file ships
 *     compile-clean; mounting is downstream.
 *
 *   - `AUTO_RUN_FOLDER_NAME` constant from
 *     `'../../../../renderer/components/Wizard/services/phaseGenerator'` —
 *     pure string constant.
 *
 *   - `ScreenReaderAnnouncement` from `'../ScreenReaderAnnouncement'`
 *     (already in webFull from Phase 1).
 *
 *   - `DocumentEditor` from `'../shared/DocumentEditor'` (lifted in this
 *     batch).
 *
 *   - `formatShortcutKeys` from `'../../../utils/shortcutFormatter'`
 *     (already in webFull).
 *
 * No host wiring; this lift makes the screen surface available in webFull
 * without yet mounting it.
 */

import { useEffect, useCallback, useRef, useState } from 'react';
import { Loader2, Rocket, Compass, X } from 'lucide-react';
import type { Theme } from '../../../../shared/theme-types';
import { useWizard } from '../../../../renderer/components/Wizard/WizardContext';
import { AUTO_RUN_FOLDER_NAME } from '../../../../renderer/components/Wizard/services/phaseGenerator';
 ...
 * `window.maestro.autorun.writeDoc(folderPath, filename, content)`. The route
 * flattens the tuple to `{path: <folderPath>/<filename>, content}` and
 * mkdir -p's parents on demand. Mirrors AutoRun's `fetchWriteDoc`.
 */
...
/**
 * Fetch helper — replacement for
 * `window.maestro.autorun.deleteImage(folderPath, relativePath)`. Mirrors the
 * AutoRun port. Uses the query-string variant of the route's dual-shape
 * contract.
 */
...
function DocumentReview({
	...
	useEffect(() => {
		...
		autoSaveTimeoutRef.current = setTimeout(async () => {
			...
			if (localContent !== lastSavedContentRef.current) {
				...
				try {
					// `autorun.writeDoc` swap → `/api/autorun/write-doc`.
					await fetchWriteDoc(folderPath, currentDoc.filename, localContent);
					...
					if (
						...
						try {
							isSavingRef.current = true;
							// `autorun.writeDoc` swap → `/api/autorun/write-doc`.
							await fetchWriteDoc(folderPath, currentDoc.filename, pendingContent);
	...
	const handleRemoveAttachment = useCallback(
		async (filename: string) => {
			...
			// `autorun.deleteImage` swap → `/api/autorun/delete-image`. Same string
			// the renderer-side IPC accepted as `filename` is sent on the route as
			// `relativePath` (route's parameter rename — same value).
			await fetchDeleteImage(folderPath, filename);
