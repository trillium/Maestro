/**
 * PreparingPlanScreen.tsx (webFull lift — Phase 4)
 *
 * Lifted verbatim from
 * `src/renderer/components/Wizard/screens/PreparingPlanScreen.tsx` (~914 LOC)
 * per ISC-44.lift.wizard_preparing_plan_screen_lifted. Document-generation
 * screen of the onboarding wizard — automatic, no user input, advances to
 * review when generation completes.
 *
 * Imports repointed to webFull-resident copies:
 *
 *   Renderer source                                      webFull lift
 *   ───────────────────────────────────────────────────  ─────────────────────
 *   `import type { Theme } from '../../../types'`        `import type { Theme }
 *                                                         from '../../../../shared/theme-types'`
 *   `import { useWizard } from '../WizardContext'`       `from '../contexts/WizardContext'`
 *   `import { phaseGenerator, wizardDebugLogger,         `from '../services/phaseGenerator'`
 *     type CreatedFileInfo } from '../services/...'`     (webFull module now
 *                                                         exports a Proxy
 *                                                         `phaseGenerator`
 *                                                         backed by
 *                                                         `setPhaseGenerator`
 ...
 *   `import { getNextAustinFact, parseFactWithLinks,     `from '../services/austinFacts'`
 *     type FactSegment } from '../services/austinFacts'`
 *   `import { formatSize, formatElapsedTime }            `from '../../../../shared/formatters'`
 *     from '../../../../shared/formatters'`
 *
 * IPC swap (1 site):
 *
 *   `if (!window.maestro?.shell?.openExternal?.(url)) {  `window.open(url, '_blank',
 *      window.open(url, '_blank');                        'noopener,noreferrer');`
 *    }`
 *
 *   The renderer already had a `window.open` fallback because the call site
 *   sits inside a presentational component shared with non-Electron contexts.
 *   In webFull we drop the IPC branch entirely and always use `window.open`,
 *   matching the precedent set in `Wizard/shared/DocumentEditor.tsx` and
 *   `InlineWizard/WizardMessageBubble.tsx`.
 *
 * Singleton wiring note:
 *
 *   The renderer's `phaseGenerator` was a no-arg module-level singleton. The
 *   webFull `PhaseGenerator` requires `serverToken` + `processLifecycleClient`
 *   in its constructor, so the host wires it once per session via
 *   `setPhaseGenerator(createPhaseGenerator(config))`. Until that call
 *   happens, every `phaseGenerator.*` call throws with an actionable
 *   message — same posture as `setProcessLifecycleClient` /
 *   `setAgentResolver` in `conversationManager.ts`. This screen reads the
 *   singleton lazily through the proxy, so import-order does not matter.
 *
 * No host wiring is added in this lift; the screen is wire-ready but not
 * mountable until a host installs the wizard ecosystem (a future phase, same
 * as PhaseReviewScreen).
 *
 * ----------------------------------------------------------------------------
 * Original component docstring (preserved):
 *
 ...
 * Links open in a new browser tab via `window.open(href, '_blank',
 * 'noopener,noreferrer')`. The renderer's `window.maestro.shell.openExternal`
 * branch is dropped in webFull (see file-level docstring).
 */
				...
				if (isFullyVisible) {
					...
					elements.push(
						<a
							...
							onClick={(e) => {
								e.preventDefault();
								// Open in a new browser tab (webFull has no Electron shell)
