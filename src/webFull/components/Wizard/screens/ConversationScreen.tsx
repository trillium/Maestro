/**
 * ConversationScreen.tsx (webFull lift — Phase 4 conversation surface)
 *
 * Lifted from `src/renderer/components/Wizard/screens/ConversationScreen.tsx`
 * per `ISC-44.lift.wizard_conversation_screen_lifted`. Third screen of the
 * onboarding wizard — AI-driven conversation for project discovery with
 * confidence meter and structured output parsing.
 *
 * IPC swaps (4 direct callsites — both backing endpoints stubbed pending
 * server routes):
 *
 *   1. `window.maestro.autorun.listDocs(folderPath)` (×2 — once in
 *      `initConversation` → `fetchExistingDocs`, once in
 *      `sendInitialContinueMessage`) → TODO: `GET /api/autorun/list-docs?path=…`
 *      is not yet implemented on the webFull server. The stub logs a warning
 *      and returns `{ docs: [] }` so the resume / continue-with-existing-docs
 *      flow is skipped gracefully on webFull until the route ships.
 *
 *   2. `window.maestro.autorun.readDoc(folderPath, filename)` (×2 — both
 *      invoked inside the same loops that walk `listDocs`) → TODO:
 *      `GET /api/autorun/read-doc?path=…&filename=…` is not yet implemented.
 *      Stub logs a warning and returns `null` so the per-file read is no-op'd.
 *
 * Because `listDocs` returns an empty list, the `readDoc` stub is unreachable
 * at runtime today — it is kept honest (warning + null) so when the
 * `listDocs` route ships the second stub will surface the missing
 * `readDoc` route immediately.
 *
 * Cross-fork import map (renderer → webFull):
 *
 *   - `Theme` from `'../../../types'` → `'../../../../shared/theme-types'`
 *   - `WizardContext` (`useWizard`, `WizardMessage`) from `'../WizardContext'`
 *     → `'../../contexts/WizardContext'`
 *   - `wizardPrompts` from `'../services/wizardPrompts'` → `'../../services/wizardPrompts'`
 *   - `conversationManager` (incl. `createUserMessage`,
 *     `createAssistantMessage`) from `'../services/conversationManager'` →
 *     `'../../services/conversationManager'`
 *   - `WizardError` type from `'../services/wizardErrorDetection'` →
 *     `'../../services/wizardErrorDetection'`
 *   - `AUTO_RUN_FOLDER_NAME` was renderer-re-exported from
 *     `'../services/phaseGenerator'`; sourced directly from
 *     `'../../../../constants/autorun'` here (matches the precedent in
 *     `src/webFull/components/Wizard/services/phaseGenerator.ts` L48).
 *   - `wizardDebugLogger` from `'../services/phaseGenerator'` →
 *     `'../../services/wizardDebugLogger'` (single-purpose module exists in
 *     webFull; `phaseGenerator` also re-exports it but the dedicated module
 *     is the canonical path).
 *   - `fillerPhrases` from `'../services/fillerPhrases'` → `'../../services/fillerPhrases'`
 *   - `ScreenReaderAnnouncement` from `'../ScreenReaderAnnouncement'` →
 *     `'../../ScreenReaderAnnouncement'`
 *   - `formatShortcutKeys` from `'../../../utils/shortcutFormatter'` →
 *     `'../../../../utils/shortcutFormatter'`
 *   - `TypingIndicator` from `'../shared/TypingIndicator'` → `'../../shared/TypingIndicator'`
 *   - `wizardHelpers` from `'../shared/wizardHelpers'` → `'../../shared/wizardHelpers'`
 *   - `markdownConfig` (`REMARK_GFM_PLUGINS`,
 *     `createWizardBubbleMarkdownComponents`) from
 *     `'../../../../shared/utils/markdownConfig'` →
 *     `'../../../../utils/markdownConfig'` (webFull-side surgical extract
 *     that swaps Electron `shell.openExternal` for `window.open`).
 *
 * Renderer-side `ConversationScreen.tsx` is UNTOUCHED — fork hygiene.
 *
 * Features (unchanged from renderer):
...
import { useWizard, type WizardMessage } from '../../contexts/WizardContext';
import {
	getConfidenceColor,
	getInitialQuestion,
...
import { AUTO_RUN_FOLDER_NAME } from '../../../../constants/autorun';
import { wizardDebugLogger } from '../../services/wizardDebugLogger';
...
// ============================================================================
// webFull IPC stubs (TODO: pending server routes)
// ============================================================================

/**
 * Stub for `window.maestro.autorun.listDocs(folderPath)`.
 *
 * The renderer call returns `{ success, files }` where `files` is a string[]
 * of filenames in the `.maestro/auto-run/` folder. On webFull there is no
 * `/api/autorun/list-docs` route yet, so this stub returns an empty file
 * list which causes the calling code to treat the project as having no
 * existing docs and skip the "continue with existing docs" path.
 *
 * When the route lands, replace the body with a `fetch(buildApiUrl(...))`
 * call matching the renderer return shape (`{ success: boolean; files?:
 * string[]; error?: string }`).
 */
async function stubListDocs(
	folderPath: string
): Promise<{ success: boolean; files?: string[]; error?: string }> {
	console.warn('TODO: /api/autorun/list-docs not yet implemented (folderPath=' + folderPath + ')');
	return { success: true, files: [] };
}

/**
 * Stub for `window.maestro.autorun.readDoc(folderPath, filename)`.
 *
 * The renderer call returns `{ success, content }` where `content` is the
 * UTF-8 body of the file. On webFull there is no `/api/autorun/read-doc`
 * route yet, so this stub returns `null` (treated as a read failure by the
 * caller — the file is skipped via the existing `if (readResult.success &&
 * readResult.content)` guard).
 *
 * Today this stub is unreachable at runtime because `stubListDocs` returns
 * an empty file list, but it is kept honest so the next phase that lights
 * up the `listDocs` route surfaces the missing `readDoc` route too.
 */
async function stubReadDoc(
	folderPath: string,
	filename: string
): Promise<{ success: boolean; content?: string; error?: string } | null> {
	console.warn(
		'TODO: /api/autorun/read-doc not yet implemented (folderPath=' +
			folderPath +
			', filename=' +
			filename +
			')'
	...
	useEffect(() => {
		...
		async function fetchExistingDocs(): Promise<ExistingDocument[]> {
			...
			try {
				...
				const listResult = await stubListDocs(autoRunPath);
				...
				for (const filename of listResult.files) {
					try {
						const readResult = await stubReadDoc(autoRunPath, filename);
