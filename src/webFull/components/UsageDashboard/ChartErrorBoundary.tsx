/**
 * ChartErrorBoundary — Usage Dashboard
 *
 * Lifted from src/renderer/components/UsageDashboard/ChartErrorBoundary.tsx as
 * part of the Phase-1 leaf wave. Implementation is verbatim except:
 *
 * - `Theme` import path swap:
 ...
 *     matching the lifted `EmptyState`, `Modal`, and `FormInput` pattern).
 *
 * - Logger swap:
 *     Renderer: `import { logger } from '../../utils/logger'` (IPC-bound
 *     RendererLogger forwarding to `window.maestro.logger.log`).
 *     webFull:  `import { webLogger } from '../../utils/logger'` (browser
 *     console + structured prefix, no IPC dependency).
 *
 * A theme-aware React error boundary specifically designed for Usage Dashboard
 * charts. Catches JavaScript errors in chart components and displays a retry UI.
 *
 * Features:
...
export class ChartErrorBoundary extends Component<Props, State> {
	...
	componentDidCatch(error: Error, errorInfo: React.ErrorInfo) {
		...
		webLogger.error(`Chart Error Boundary: ${chartName} - ${error.message}`, 'ChartErrorBoundary', {
			chartName,
