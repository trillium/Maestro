/**
 * ActivityHeatmap — Usage Dashboard
 *
 * Lifted from src/renderer/components/UsageDashboard/ActivityHeatmap.tsx as part
 * of the Phase-1 leaf wave (Tier B — biggest pure chart). Implementation is
 * verbatim except three import-path swaps:
 *
 * - `Theme`:  `'../../types'` → `'../../../shared/theme-types'`
 *   (matches `EmptyState` / `ChartErrorBoundary` pattern; no `types/`
 *   aggregator in webFull yet).
 * - `StatsTimeRange` / `StatsAggregation`:
 *   `'../../hooks/stats/useStats'` → `'./types'`
 *   (Phase-1 lifts the chart leaves ahead of the Phase-3 engine hook. We
 *   extract the data shapes into a sibling `types.ts` so charts can land
 *   without pulling the IPC-bound `useStats` orchestrator into webFull. When
 *   the hook is ported in Phase 3, it should import these from `./types`.)
 * - `COLORBLIND_HEATMAP_SCALE`:
 *   `'../../constants/colorblindPalettes'` → `'../../constants/colorblindPalettes'`
 *   (path string identical, but resolves to the lifted
 *   `src/webFull/constants/colorblindPalettes.ts`, which is a verbatim copy
 *   of the renderer constants module — same exports, same values).
 *
