 ...
 * MermaidRenderer
 *
 * Lifted verbatim from `src/renderer/components/MermaidRenderer.tsx` as part
 * of the Layer 2.5 leaf-parade lift wave (paired with the `GroupChatModal`
 * pre-flight that landed STOPPED on the same branch — see ISA Decision
 * 2026-06-08, branch `leaf-mermaid-and-groupchat-modal`).
 *
 * Pre-flight gate (per the brief's "Pre-flight each; STOP that one if
 * module-load IPC" rule):
 *   $ grep -nE "window\.maestro\.|window\.electron|ipcRenderer|from ['\"]electron['\"]|shell\.openExternal|shell\.openPath" \
 *       src/renderer/components/MermaidRenderer.tsx
 *   (empty — exit 1)
 *
 * 0 IPC namespaces touched. 0 Electron-only APIs touched. The component is
 * a pure consumer of three module deps that already work in the browser:
 *   - `mermaid` — pure DOM-rendering library (already a webFull-tree dep
 *     used by `MarkdownRenderer` at the L2.5 lift wave; `useMaxWidth: true`
 *     + `securityLevel: 'strict'` keep it sandbox-friendly)
 *   - `dompurify` — pure SVG sanitiser (already a webFull-tree dep)
 *   - `react` — React hooks (`useLayoutEffect`, `useRef`, `useState`)
 *
 ...
 *     canonical type directly to avoid a silent-drift surface (audit risk
 *     A from the 2026-06-08 Architect re-eval).
 *
 * Theme access pattern: keeps the renderer's `theme: Theme` prop convention,
 * consistent with the L2.1 / L2.3 / L2.4 / L2.5 precedents. Callers in
 * webFull call `const { theme } = useTheme()` at the feature-component
 * level and thread it down.
 *
 * Composition: this is a fenced-code-block renderer, NOT a modal — no
 * Modal / ModalFooter / layer-stack registration. Self-contained: clears
 * a module-scoped `lastThemeId` between renders so the mermaid singleton
 * re-initialises on theme swaps and reuses the cached theme variables
 * otherwise.
 *
 * Mermaid singleton caveat: `lastThemeId` is module-scoped. If two
 * `MermaidRenderer` instances mount with different themes simultaneously,
 * the last initialiser wins — same constraint as the renderer source.
 * This is acceptable for the use case (one chart at a time per active tab)
 * and the renderer parity catalog has not asserted multi-instance
 * cross-theme behaviour either.
 *
 * Theme variable budget: ~260 LOC of colour-palette math is preserved
 * verbatim — every flowchart / sequence / gantt / pie / git-graph /
 * mindmap / quadrant / xy-chart / timeline / sankey / state / ER /
 * requirement / class diagram surface mermaid exposes is tinted with the
 * app's accent / success / warning / error / border / textMain / textDim
 * colours so diagrams blend with the active theme. Dark-vs-light branch
 * is detected via luminance threshold on `bgMain`.
 */
