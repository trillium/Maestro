/**
 * AutoRun — webFull port (full lift, ISC-44.lift.autorun_main)
 *
 * Lifted from `src/renderer/components/AutoRun.tsx` (~2285 LOC) as the
 * head of the Layer 2.5 AutoRun-view leaf-parade. The renderer source's
 * 13 IPC sites collapse onto pre-existing W3 REST routes per the
 * `ISC-44.shim.big_3_ipc_strategy` umbrella Decision (2026-06-08). Two
 * sibling shims close in the same lift:
 *
 *   ISC-44.lift.autorun_main                     — this file
 *   ISC-44.shim.use_autorun_image_handling_webfull_port
 *     → `src/webFull/hooks/useAutoRunImageHandling.ts` (sibling port,
 *       fetch-backed mirror of the renderer hook)
 *
 * History note: prior leaf-autorun attempts STOP-and-retried on transitive
 * audit-correction surfaces — the `fs:readFile` image branch (closed by
 * the W3 `/api/fs/read-image` route landing in `1b2ae0a53`) and the hook
 * transitive consumer (closed by the sibling webFull port). This lift is
 * the third attempt; all prerequisite server routes are now live on
 * `origin/main` `1b2ae0a53`:
 *
 *   - GET    /api/fs/read-file          (text, 400s on binary)
 *   - GET    /api/fs/read-image         (data:image/...;base64,... bare text)
 *   - POST   /api/autorun/write-doc     (body `{path, content}`)
 *   - GET    /api/autorun/list-images   (query `folderPath`, `docFilename`)
 *   - POST   /api/autorun/save-image    (body `{folderPath, docFilename, dataUrl, extension}`)
 *   - DELETE /api/autorun/delete-image  (query or body `{folderPath, relativePath}`)
 *
 * ## IPC site → REST / browser-API mapping (13 sites total)
 *
 *   AutoRun.tsx:265 fs.readFile(absolutePath, sshRemoteId)         → GET /api/fs/read-image
 *   AutoRun.tsx:285 fs.readFile(src, sshRemoteId)                  → GET /api/fs/read-image
 *   AutoRun.tsx:314 fs.readFile(pathToLoad, sshRemoteId)           → GET /api/fs/read-image
 *   AutoRun.tsx:653 autorun.writeDoc(folderPath, name+'.md', ...)  → POST /api/autorun/write-doc
 *   AutoRun.tsx:748 autorun.writeDoc(folderPath, name+'.md', ...)  → POST /api/autorun/write-doc
 *   AutoRun.tsx:1467 shell.openExternal(href)                      → window.open(href, '_blank', 'noopener,noreferrer')
 *   AutoRun.tsx:1520 shell.openExternal(href)                      → window.open(href, '_blank', 'noopener,noreferrer')
 *   useAutoRunImageHandling.ts:157 autorun.listImages(...)         → (sibling port) GET /api/autorun/list-images
 *   useAutoRunImageHandling.ts:175 fs.readFile(absolutePath, ...)  → (sibling port) GET /api/fs/read-image
 *   useAutoRunImageHandling.ts:278 autorun.saveImage(...)          → (sibling port) POST /api/autorun/save-image
 *   useAutoRunImageHandling.ts:368 autorun.saveImage(...)          → (sibling port) POST /api/autorun/save-image
 *   useAutoRunImageHandling.ts:419 autorun.deleteImage(...)        → (sibling port) DELETE /api/autorun/delete-image
 *   useAutoRunImageHandling.ts:484 autorun.deleteImage(...)        → (sibling port) DELETE /api/autorun/delete-image
 *
 * All 7 AutoRun.tsx sites are rewritten in-place. All 6 hook sites are
 * served by the sibling webFull port at `'../hooks/useAutoRunImageHandling'`.
 *
 * ## Strip-and-promote-to-prop adapt — settings store
 *
 * The renderer source self-sources three bionify selectors from
 * `useSettingsStore`:
 *
 ...
 *
 * Following the established L2.5 precedent (TerminalOutput, GroupChatInput,
 * AppOverlays, HistoryDetailModal), `useSettingsStore` is renderer-only
 * (110+ IPC sites) and the three selectors are stripped + promoted to
 * optional props:
 *
 *   bionifyReadingMode?: boolean   (default false)
 *   bionifyIntensity?:   number    (default 1)
 *   bionifyAlgorithm?:   string    (default '- 0 1 1 2 0.4')
 *
 * The host wires these from whatever settings surface is available
 * (`useSettings()` REST hook, WebSocket snapshot, etc.).
 *
 * ## Wire-shape parity for inline edits
 *
 *   fs.readFile(path, sshRemoteId)  ───────  ALL the AutoRun.tsx sites pass IMAGE paths
 *     The 3 fs.readFile sites are all inside the `AttachmentImage` component
 *     and all targets are image files inside the AutoRun folder (.png / .jpg etc.).
 *     They are rewritten to use the bare-text `/api/fs/read-image` endpoint
 *     (`await fetch(...).then(r => r.text())`) which returns a
 *     `data:image/...;base64,...` string the renderer expects via
 *     `result.startsWith('data:')`.
 *
 *   autorun.writeDoc(folderPath, filename, content, sshRemoteId)
 *     The route signature is FLATTENED — `{path, content}` where the client
 *     constructs `path = ${folderPath}/${filename}`. Both AutoRun.tsx call
 *     sites already pass `selectedFile + '.md'` so the `.md` suffix is in
 *     the filename. SSH parity: the route 501s on SSH — the renderer side
 *     handles SSH; webFull-mounted SSH sessions remain out of scope per
 *     the established W3 pattern (audit risk acknowledged).
 *
 *   shell.openExternal(href)
 *     Browser-native `window.open(href, '_blank', 'noopener,noreferrer')`
 *     replaces the Electron-only IPC. The two sites both already guard on
 *     the URL scheme (`/^https?:\/\/|^mailto:/`); we preserve the guard
 *     verbatim and only swap the call.
 *
 * ## Import-path adjustments
 *
 *   1. `'../utils/tokenCounter'` → `'../../renderer/utils/tokenCounter'`
 *      (CROSS-FORK EDGE — pure GPT/Claude tokenizer, no IPC, no DOM).
 *
 *   2. `BatchRunState, SessionState, Theme, Shortcut` from `'../types'` →
 *      `Theme` from `'../../shared/theme-types'` + the rest from
 *      `'../../renderer/types'`. Standard L2.5 swap — `Theme` is the
 *      canonical shared type; the other three are pure data shapes from
 *      the renderer types barrel (no IPC reach).
 *
 *   3. `FileNode` from `'../types/fileTree'` →
 *      `'../../renderer/types/fileTree'` (CROSS-FORK EDGE — pure tree
 *      data shape, no IPC).
 *
 *   4. Sibling components (`AutoRunnerHelpModal`, `ResetTasksConfirmModal`,
 *      `MermaidRenderer`, `AutoRunDocumentSelector`, `AutoRunLightbox`,
 *      `AutoRunSearchBar`, `TemplateAutocompleteDropdown`) — all already
 *      ported into `src/webFull/components/` in earlier L2.5 lifts; paths
 *      unchanged.
 *
 *   5. `useTemplateAutocomplete`, `useAutoRunUndo` from `'../hooks'` →
 *      `'../../renderer/hooks'` (CROSS-FORK EDGE — both pure renderer hooks
 *      with zero IPC reach, verified by grep). `useAutoRunImageHandling` +
 *      `imageCache` from the LOCAL sibling port at
 *      `'../hooks/useAutoRunImageHandling'`. This is the critical break
 *      from the renderer hooks barrel — the sibling port wires the four
 *      REST routes that replace the hook's IPC sites.
 *
 *   6. `REMARK_GFM_PLUGINS, generateAutoRunProseStyles, createMarkdownComponents`
 *      from `'../utils/markdownConfig'` →
 *      `'../../renderer/utils/markdownConfig'` (CROSS-FORK EDGE). Module
 *      has 2 `window.maestro.shell.openExternal` sites at lines 696, 796
 *      inside its DEFAULT `a` renderer. AutoRun passes an explicit
 *      `onExternalLinkClick` callback through both `createMarkdownComponents`
 *      calls (verified at lines 1465-1469 and 1518-1522), which overrides
 *      the default `a` renderer and routes through our callback instead
 *      (our callback uses `window.open()`). So the transitive IPC sites
 *      are unreachable at runtime from this component.
 *
 *   7. `formatShortcutKeys` from `'../utils/shortcutFormatter'` → kept
 *      relative (webFull has its own mirror at
 *      `src/webFull/utils/shortcutFormatter.ts`).
 *
 *   8. `remarkFileLinks, buildFileTreeIndices` from `'../utils/remarkFileLinks'`
 *      → `'../../renderer/utils/remarkFileLinks'` (CROSS-FORK EDGE — pure
 *      remark plugin, no IPC reach).
 *
 *   9. `useBatchStore` from `'../stores/batchStore'` →
 *      `'../../renderer/stores/batchStore'` (CROSS-FORK EDGE — pure
 *      Zustand store, 0 IPC sites verified by grep; this component only
 *      READS from it).
 *
 *  10. `useSettingsStore` import — REMOVED. Replaced by props (see
 *      strip-and-promote section above).
 *
 * ## Pure browser runtime
 *
 * After all edits: 0 `window.maestro.*` references at module load or
 * runtime in this file. `grep "window.maestro" src/webFull/components/AutoRun.tsx`
 * returns zero hits. The only `window.*` references are `window.open`
 * (browser-standard) and `window.location` (none here, but allowed).
 */
import {
	useState,
	useRef,
...
interface AutoRunProps {
	...
	// ============================================================================
	// Bionify reading mode — promoted to props in the webFull lift (the renderer
	// source self-sources these from `useSettingsStore`; the store carries 110+
	// `window.maestro.settings.set` IPC sites and is renderer-only as of this
	// lift). Optional with renderer-side store defaults so a host that doesn't
	// wire them through still gets the renderer's baseline behavior. See lift
	// docblock at top of file for context.
	// ============================================================================
...
// Fetch helper — replacement for `window.maestro.fs.readFile(path, sshRemoteId)`
// in the AttachmentImage component. Calls the W3 `/api/fs/read-image` route
// which returns a bare-text `data:image/<ext>;base64,<payload>` URL string
// (matches the renderer-side IPC reply shape byte-for-byte). Resolves to the
// data URL on success, throws on HTTP error (mirrors the renderer Promise's
// throw-on-error semantic so the existing `.catch(...)` chain in the
// AttachmentImage component works unchanged).
async function fetchReadImageForAttachment(absolutePath: string): Promise<string> {
	const url = buildApiUrl('/fs/read-image') + `?path=${encodeURIComponent(absolutePath)}`;
	const res = await fetch(url);
	if (!res.ok) {
		// Surface a tight error message matching what the renderer's `.catch`
		// chain would see; the AttachmentImage `.catch` uses `err.message`.
...
// Fetch helper — replacement for
// `window.maestro.autorun.writeDoc(folderPath, filename, content, sshRemoteId)`
// at the two save sites. The route signature FLATTENS the tuple to a single
// absolute path: `{path: <folderPath>/<filename>, content}`. Both call sites
// already pass `selectedFile + '.md'` so the suffix is included client-side.
// The route mkdir -p's parent directories on demand (matches the renderer
// IPC path). sshRemoteId is NOT supported on the route — the renderer side
// owns SSH; webFull SSH AutoRun remains out of scope.
async function fetchWriteDoc(folderPath: string, filename: string, content: string): Promise<void> {
	const absolutePath = `${folderPath}/${filename}`;
	const res = await fetch(buildApiUrl('/autorun/write-doc'), {
		method: 'POST',
...
const AttachmentImage = memo(function AttachmentImage({
	...
	useEffect(() => {
		...
		if (decodedSrc.startsWith('images/')) {
			...
			// Load from folder using absolute path — webFull lift: replaces
			// `window.maestro.fs.readFile(absolutePath, sshRemoteId)` with the
			// W3 `/api/fs/read-image` route. The route returns the same
			// `data:image/<ext>;base64,...` bare-text payload as the renderer
			// IPC, so the downstream `.startsWith('data:')` check is unchanged.
			// sshRemoteId is NOT forwarded — the route 501s on it; webFull-side
			// SSH AutoRun support remains out of scope per the W3 contract.
			const absolutePath = `${folderPath}/${decodedSrc}`;
			fetchReadImageForAttachment(absolutePath)
				.then((result) => {
					if (isStale) return;
			...
			// Absolute file path — webFull lift: same swap as the relative
			// branch above. The route validates `path` is an absolute path
			// inside the configured Auto Run folder allowlist (see
			// `validateFsPath()` in `src/main/web-server/routes/apiRoutes.ts`).
			setFilename(src.split('/').pop() || null);
			fetchReadImageForAttachment(src)
				.then((result) => {
					if (isStale) return;
					if (result && result.startsWith('data:')) {
						setDataUrl(result);
			...
			// Other relative path — webFull lift: same swap as the two branches
			// above. The renderer source uses `fs.readFile` here too because
			// all branches resolve to image-data lookups inside the Auto Run
			// folder. The `/api/fs/read-image` extension allowlist gates the
			// payload to image MIMEs.
			const pathToLoad = `${folderPath}/${src}`;
			fetchReadImageForAttachment(pathToLoad)
				.then((result) => {
					if (isStale) return;
					if (result && result.startsWith('data:')) {
						imageCache.set(cacheKey, result);
...
const AutoRunInner = forwardRef<AutoRunHandle, AutoRunProps>(function AutoRunInner(
	{
		...
		onShowFlash,
		// Bionify reading-mode props — webFull lift strip-and-promote. Defaults
		// mirror the renderer-side `useSettingsStore` initial values verbatim
	...
	// Bionify values come from props in the webFull lift (strip-and-promote
	// from `useSettingsStore` — see lift docblock at top of file).
	...
	const handleSave = useCallback(async () => {
		...
		try {
			// webFull lift: replaces `window.maestro.autorun.writeDoc(folderPath,
			// filename, content, sshRemoteId)` with POST /api/autorun/write-doc.
			// `selectedFile + '.md'` is constructed client-side as in the
			// renderer source; the route's flattened `{path, content}` shape
			// reassembles the absolute path inside `fetchWriteDoc`. sshRemoteId
			// is dropped — the route 501s on it and the Electron path owns SSH.
			await fetchWriteDoc(folderPath, selectedFile + '.md', localContent);
	...
	const handleResetTasks = useCallback(async () => {
		...
		// Auto-save the reset content — webFull lift: same swap as the
		// `handleSave` site above. POST /api/autorun/write-doc with the
		// flattened `{path, content}` shape. sshRemoteId dropped per the
		// route's no-SSH contract.
		try {
			await fetchWriteDoc(folderPath, selectedFile + '.md', resetContent);
	...
	const baseMarkdownComponents = useMemo(() => {
		const components = createMarkdownComponents({
			...
			// Open external links in system browser — webFull lift: replaces
			// `window.maestro.shell.openExternal(href)` with the browser-native
			// `window.open(href, '_blank', 'noopener,noreferrer')`. The URL
			// scheme guard (`/^https?:\/\/|^mailto:/`) is preserved verbatim.
			// `noopener` + `noreferrer` block the new tab from reaching back
			// into the opener (XSS / tabnabbing defense).
	...
	const searchHighlightedComponents = useMemo(() => {
		...
		const components = createMarkdownComponents({
			...
			bionifyAlgorithm,
			onFileClick: handleFileClick,
			// webFull lift: second `shell.openExternal` site (the
			// search-highlighted markdown variant). Same `window.open` swap
			// as the unhighlighted variant above.
			onExternalLinkClick: (href) => {
				if (/^https?:\/\/|^mailto:/.test(href)) {
					window.open(href, '_blank', 'noopener,noreferrer');
				}
