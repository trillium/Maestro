 ...
 *
 * Lifted verbatim from `src/renderer/components/TransferProgressModal.tsx`
 * (541 LOC, 0 IPC, 0 Electron-only API per pre-flight grep) as part of the
 * Layer 2.5 leaf-parade wave. Modal-shape cross-agent context transfer
 * progress surface: a centered, blocking `role="dialog"` with focus trap,
 * layer-stack registration, and an Escape-handler that shows a
 * cancel-confirmation sub-overlay. Surfaces a source → target agent
 * indicator row (`sourceAgent` / `targetAgent` display names with an
 * `ArrowRight` between), a centered Wand2 spinner head, a status title
 * row (with elapsed-time chip), an animated progress bar with percentage,
 * a four-stage vertical indicator list (`collecting` → `grooming` →
 * `creating` → `complete`) where the active labels interpolate the target
 * agent display name, and a footer Cancel/Done button that flips style on
 * completion.
 *
 * Pre-flight `grep -E "window\.maestro\.|from ['\"]electron['\"]|shell\.openExternal|shell\.openPath|ipcRenderer" src/renderer/components/TransferProgressModal.tsx`
 ...
 * adjustments matching the sibling modal lifts in this wave
 * (`MergeProgressModal`, `SummarizeProgressModal`) plus one extra route for
 * the `getAgentDisplayName` helper:
 *
 ...
 *
 * 2. `ToolType` from `'../types'` → `'../../shared/types'`. `ToolType` is
 *    declared in `src/shared/types.ts` as `type ToolType = AgentId` (a pure
 *    type-only re-export); the renderer barrel re-exports it without
 *    transform. Pulling directly from `src/shared/` matches the
 *    `AgentErrorModal` precedent.
 *
 * 3. `GroomingProgress` from `'../types/contextMerge'` →
 ...
 *    via cross-fork import matches the sibling overlay/modal precedent
 *    (audit risk A: no duplicate type modules into webFull).
 *
 ...
 *
 * 5. `MODAL_PRIORITIES` resolves via the webFull re-export at
 ...
 *    to prevent silent drift). Uses `MODAL_PRIORITIES.TRANSFER_PROGRESS`
 *    (683).
 *
 * 6. `getAgentDisplayName` from `'../services/contextGroomer'` →
 *    `'../../shared/agentMetadata'`. The renderer-side `contextGroomer.ts`
 *    is IPC-bearing (line 281: `window.maestro.context.groomContext`, line
 *    397: `window.maestro.context.cleanupGroomingSession`), so routing
 *    webFull through it would drag a `window.maestro` module-load surface
 *    into the lifted modal. The renderer's `getAgentDisplayName` is itself
 *    a re-export of the pure helper that lives in
 *    `src/shared/agentMetadata.ts` (line 16: `import { getAgentDisplayName
 *    as getDisplayName } from '../../shared/agentMetadata'`). Pulling the
 *    helper directly from `src/shared/` matches the `AgentErrorModal` /
 *    `AgentPromptComposerModal` precedent of routing pure shared helpers
 *    around the renderer barrel and avoids the IPC surface entirely.
 *
 ...
 * absolute-positioned cancel-confirmation sub-overlay, and an
 * AgentTransferIndicator pill row above the spinner. Lift is verbatim to
 ...
 * trap, TRANSFER_PROGRESS priority).
 *
 * `lucide-react` icons (`X`, `Check`, `Loader2`, `AlertTriangle`,
 * `ArrowRight`, `Wand2`) kept verbatim — already a webFull-tree dep used
