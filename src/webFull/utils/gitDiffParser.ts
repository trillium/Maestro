/**
 * gitDiffParser — webFull lift
 *
 * Layer 2.5 leaf-parade companion lift of
 * `src/renderer/utils/gitDiffParser.ts` (112 LOC, pure util — 0 IPC, 0
 * Electron-only API, 0 React surface). Lifted verbatim alongside
 * `GitDiffViewer` to avoid a cross-fork edge for the parser, which is a
 * leaf-of-the-leaf and consumed nowhere else in webFull today.
 *
 * Pre-flight `grep -E "window\.maestro\.|from ['\"]electron['\"]"
 * src/renderer/utils/gitDiffParser.ts` returned empty (exit 1). The util
 * imports only `parseDiff` / typed shapes from `react-diff-view`, which is a
 * dependency already in `package.json` and is renderer-agnostic.
 *
 * Lift policy: verbatim. No import-path adjustments needed (`react-diff-view`
 * resolves identically from both fork-roots).
 */
