/**
 * sessionValidation — webFull lift
 *
 * Verbatim port of `src/renderer/utils/sessionValidation.ts` (132 LOC,
 * pure functions — zero IPC, zero side effects). Lifted as a leaf
 * dependency of `NewInstanceModal`'s webFull port (the "create a new
 * agent" entry point, the biggest single-modal user-felt unlock per
 * the leaf-parade brief 2026-06-08).
 *
 * ## Why verbatim
 *
 * The renderer source is pure data-shape validation — name uniqueness
 * (hard error) and directory-conflict detection per host (soft warning).
 * The only changes vs the renderer:
 *
 * - `Session, ToolType` from `'../types'` → `'../../renderer/types'`
 *   (type-only re-import, established webFull pattern — see
 *   SendToAgentModal.tsx line 108).
 * - `getAgentDisplayName` from `'../../shared/agentMetadata'` →
 *   `'../../shared/agentMetadata'` (unchanged path; shared module
 *   reachable from both forks at the same relative depth).
 *
