...
| Watch external agent activity    | `src/main/storage/external-session-coordinator.ts`, `src/main/storage/session-file-watcher.ts`, `src/main/ipc/handlers/external-sessions.ts`                                                                                                                     |
| Add storage watch spec for agent | extend `BaseSessionStorage.getStorageWatchSpec()` in `src/main/storage/<agent>-session-storage.ts`                                                                                                                                                               |
...
Maestro watches each agent's session-storage directory on disk and surfaces sessions it did **not** spawn (e.g., an agent the same user launched over SSH onto this host, or by running the agent CLI directly outside Maestro). Observed sessions light up the thinking pill exactly like local ones. The watch fan-out lives in `ExternalSessionCoordinator` (`src/main/storage/external-session-coordinator.ts`), which spins up one `SessionFileWatcher` per agent and forwards a coalesced snapshot to the renderer over `window.maestro.storage.onExternalActivity`. (As of this writing the observed sessions feed the thinking pill only; ingesting their usage into the stats DB / Usage Dashboard is not yet wired; see [STATS-ANALYTICS.md](docs/agent-guides/STATS-ANALYTICS.md).)

**Same-user only.** The feature relies on local filesystem permissions to read another process's session files. It observes activity from the _same_ OS user on the _same_ host. Cross-user and true SSH-remote watching are explicitly out of scope.

**Adding a new agent.** Implement `getStorageWatchSpec()` on the agent's storage class (override the `null`-returning default from `BaseSessionStorage`). JSONL-append agents (Claude, Codex, Copilot, Factory Droid) return `activityEvent: 'append'` (the default); per-message-file agents like OpenCode return `activityEvent: 'create'`. Wire-up is automatic: the coordinator iterates the storage registry and starts a watcher for every storage that returns a non-null spec. See [AGENT-INFRA.md](docs/agent-guides/AGENT-INFRA.md) for the spec shape.
