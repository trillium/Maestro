...
- [ ] ISC-42: **Principal-decision-gated.** Either accept "no real PTY rendering in browser; only parsed/structured output via existing MessageHistory" OR scope adding xterm.js + raw-byte WS multiplex (the single hardest feature; Layer 6 in `WEB_PORT_ORDER.md`).
